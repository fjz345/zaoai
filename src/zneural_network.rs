...
impl Layer {
    ...
    fn calculate_outputs(&mut self, activation_inputs: &[f32]) -> Vec<f32>{
        ...
        assert_eq!(activation_inputs.len(), self.num_in_nodes, "Num Inputs: {}, NN Num Inputs {}. Maybe data missmatch with output size?", activation_inputs.len(), self.num_in_nodes);
        ...
        for (output_node, output) in activation_outputs.iter_mut().enumerate() {
            ...
            for j in 0..activation_inputs.len() {
                self.weighted_inputs[output_node] += activation_inputs[j] * self.weights[output_node][j];
            }
    ...
    fn apply_cost_gradient(&mut self, learn_rate: f32) {
        for node_out in 0..self.num_out_nodes {
            ...
            for node_in in 0..self.num_in_nodes {
                self.weights[node_out][node_in] -= self.weights_cost_grads[node_out][node_in] * learn_rate;
            }
        }
    }
    ...
    fn update_cost_gradients(&mut self, node_cost_values: &[f32]) {
        for node_out in 0..self.num_out_nodes {
            // Weight costs
            for node_in in 0..self.num_in_nodes {
                let derivative_cost_weight = self.activation_values[node_in] * node_cost_values[node_out];
    ...
    fn calculate_hidden_layer_node_cost_values(&self, prev_layer: &Layer, prev_node_cost_values: &[f32]) -> Vec<f32>
    {
...
impl NeuralNetwork {
    ...
    pub fn learn(&mut self, training_data: &[DataPoint], epochs_size: usize, learn_rate: f32, print: Option<bool>)
    {
        ...
        for i in 0..num_epochs
        {
            ...
            if print_enabled
            {
                println!("Training...Epoch [{}/{}] @{} (#{} - #{})", i, num_epochs, epoch_step, cur_index, cur_index + epoch_step);
            }
        ...
        if(last_epoch_size >= 1)
        {
            ...
            if print_enabled
            {
                println!("Training...Epoch [{}/{}] @{} (#{} - #{})", num_epochs, num_epochs, epoch_step, cur_index, cur_index + epoch_step);
            }
            self.learn_epoch(epoch_data, learn_rate, print);
        }
    ...
    pub fn learn_old(&mut self, training_data: &[DataPoint], mut num_epochs: usize, learn_rate: f32, print: Option<bool>)
    {
        if(training_data.len() <= 0)
        {
            panic!("Learn DataPoints length was 0");
        ...
        for i in 0..num_epochs
        {
            ...
            for i in 0..self.layers.len() {
                ...
                for bias_index in 0..self.layers[i].biases.len() {
                    ...
                    let dcost =
                    self.calculate_cost(&epoch_data[..]) - original_cost;
                    self.layers[i].biases[bias_index] -= h;
    ...
    fn update_all_cost_gradients(&mut self, data_point: &DataPoint) {
        ...
        let mut node_cost_values = output_layer.calculate_output_layer_node_cost_values(&data_point.expected_outputs[..]);
        ...
        for i in (0..(self.layers.len()-1)).rev()
        {
            ...
            node_cost_values = hidden_layer.calculate_hidden_layer_node_cost_values(prev_layer, &node_cost_values);
